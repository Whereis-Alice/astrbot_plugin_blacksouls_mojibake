...
- 修复手机 QQ 可能把旧隐藏零宽可逆字符显示成可见占位符的问题。
- `codec.lossless_style = hidden` 现在改用 Unicode Tag 字符保存隐藏字节，继续保持 100% 可逆，同时更适合移动端显示。
- 保留旧零宽隐藏方案为 `codec.lossless_style = zero_width`，并保持对旧消息的解码兼容；同时兼容解码短暂测试过的变体选择符隐藏格式。

## v0.2.7

- 进一步加强爱丽丝触发词隐藏：LLM 请求阶段只会从历史上下文文本中移除配置的触发词，不会清空其他记忆内容。
- 如果某条历史只剩触发词，会替换成中性占位 `（唤醒词已隐藏）`。
