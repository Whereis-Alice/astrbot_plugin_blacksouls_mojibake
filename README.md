...
如果 AstrBot 把 `/` 配成了唤醒前缀，框架可能会先把 `/` 从消息里剥掉。插件会同时识别 `/奈亚语` 和 `奈亚语` 这两种形式，所以默认命令仍然可用。
...
bot 仍然会按原本的人格、上下文和模型能力正常生成回复。触发词只负责唤醒 LLM，插件会在发给 LLM 前把触发词从用户消息里隐藏掉，避免人格把“爱丽丝里德尔”看成用户在复读。

触发词会在事件阶段就从 `event.message_str` 里剥离；LLM 请求阶段还会从历史上下文文本里移除配置的触发词，避免旧历史继续让人格看到这几个字。它不会清空上下文，其他记忆内容会保留；如果某条历史只剩触发词，会变成 `（唤醒词已隐藏）`。
...
插件只会在 LLM 回复完成后，把这条回复转换成奈亚语再发出去。
...
```text
display.include_original_text = true
```
...
这个设置会影响中文转奈亚语命令、LLM 工具转奈亚语，以及爱丽丝触发后的奈亚语回复。
...
- `include_original_text`：奈亚语回复后是否附带原中文。开启后格式为 `奈亚语（原中文）`。
...
触发词会在 LLM 请求前被隐藏。如果消息里有触发词之外的内容，例如 `爱丽丝里德尔 你好`，LLM 只会看到 `你好`。
...
- `lossless_style`：可逆信息的保存方式。默认 `hidden`，使用 Unicode Tag 字符隐藏字节，手机 QQ 上比旧零宽字符更不容易露出占位符；`visible` 会显示成 `・84` 这类文本标记；`zero_width` 是旧隐藏方案，不推荐手机端使用。
...
原始 BLACKSOULS 乱码有时会丢字节，所以并不总是可逆。命令和 LLM 工具默认启用“可逆奈亚语”，并且默认用隐藏 Unicode Tag 字符保存字节信息。视觉上大致像这样：
...
实际文本里 `・` 后面带有用户看不见的 Tag 字符，用来保存原始字节，所以工具可以精确还原。旧版 `zero_width` 模式使用零宽字符，在部分手机 QQ 字体里可能会显示成奇怪占位符；现在默认的 `hidden` 已经避开这个问题。
...
注意：隐藏模式依赖聊天平台保留不可见字符。如果平台或复制过程把这些字符过滤掉，仍然会退化成旧式不可逆乱码。
