...
爱丽丝触发回复默认使用更像原始 BLACKSOULS 的非可逆奈亚语，避免出现大量 `・83`、`・EF` 这种十六进制标记。需要精确可逆时，可以打开 `alice_trigger.lossless_encode`。
...
原始 BLACKSOULS 乱码有时会丢字节，所以并不总是可逆。命令和 LLM 工具默认启用“可逆奈亚语”：
...
这样看起来仍然像乱码，但工具可以精确还原。`83`、`84`、`85`、`EF` 这类内容是原始 UTF-8 字节的十六进制标记，用来保证可逆。
